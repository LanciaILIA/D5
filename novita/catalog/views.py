...
class CategoryListView(generic.ListView): # D6
    model = Category
    # ordering = 'name'
    ...
    def get_queryset_(self): # сортировка по дате
        queryset = Category.objects.all().order_by('-data')
        return queryset
...
def subscrlbe(request, pk): # подписка на выбранную категорию
    user = request.user # текущий пользователь
    category = Category.objects.get(id=pk) # выбранная категория
    category.subscrlbers.add(user)

    message = 'Вы успешно подписались на рассылку новостей категории'
    return render(request, 'news/subscrlbe.html', {'category': category, 'message': message})